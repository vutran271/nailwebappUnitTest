...
namespace nailmvc2.Controllers.Tests
{
    public class ReviewControllerTests
    {
        ...
        public void ReviewControllerTest2 (int id , object expected)
        {
            var CustomersList = new List<CustomerInfomation>()
            {
                new CustomerInfomation()
                {
                    CustomerName = " Test cusomer 1",
                    CustomerID = 661166,
                }
            }.AsQueryable();
            var DbSetCustomer = new Mock<DbSet<CustomerInfomation>>();
            DbSetCustomer.As<IQueryable>().Setup(s => s.Provider).Returns(CustomersList.Provider);
            DbSetCustomer.As<IQueryable>().Setup(s => s.Expression).Returns(CustomersList.Expression);
            DbSetCustomer.As<IQueryable>().Setup(s => s.ElementType).Returns(CustomersList.ElementType);

            var EntitiesObject = new Mock<Entities>();
            EntitiesObject.Setup(s => s.CustomerInfomations).Returns(DbSetCustomer.Object);

            var Re = new ReviewController(EntitiesObject.Object);
